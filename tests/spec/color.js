describe("Color", () => {
	describe("add", () => {
		...
		it("should add three colors", () => {
			expect(ROT.Color.add([1, 2, 3], [3, 4, 5], [100, 200, 300])).toEqual([
				104, 206, 308,
			]);
		});
		it("should add one color (noop)", () => {
			expect(ROT.Color.add([1, 2, 3])).toEqual([1, 2, 3]);
		});

		it("should not modify first argument values", () => {
			var c1 = [1, 2, 3];
			var c2 = [3, 4, 5];
			ROT.Color.add(c1, c2);
	...
	describe("add_", () => {
		...
		it("should add three colors", () => {
			expect(ROT.Color.add_([1, 2, 3], [3, 4, 5], [100, 200, 300])).toEqual([
				104, 206, 308,
			]);
		});
		it("should add one color (noop)", () => {
		...
		it("should modify first argument values", () => {
			var c1 = [1, 2, 3];
			var c2 = [3, 4, 5];
	...
	describe("multiply", () => {
		it("should multiply two colors", () => {
			expect(ROT.Color.multiply([100, 200, 300], [51, 51, 51])).toEqual([
				20, 40, 60,
			]);
		});
		it("should multiply three colors", () => {
			expect(
				ROT.Color.multiply([100, 200, 300], [51, 51, 51], [510, 510, 510]),
			).toEqual([40, 80, 120]);
		});
		it("should multiply one color (noop)", () => {
		...
		it("should round values", () => {
			expect(ROT.Color.multiply([100, 200, 300], [10, 10, 10])).toEqual([
				4, 8, 12,
			]);
		});
	});
	...
	describe("multiply_", () => {
		it("should multiply two colors", () => {
			expect(ROT.Color.multiply_([100, 200, 300], [51, 51, 51])).toEqual([
				20, 40, 60,
			]);
		});
		it("should multiply three colors", () => {
			expect(
				ROT.Color.multiply_([100, 200, 300], [51, 51, 51], [510, 510, 510]),
			).toEqual([40, 80, 120]);
		});
		it("should multiply one color (noop)", () => {
		...
		it("should round values", () => {
			expect(ROT.Color.multiply_([100, 200, 300], [10, 10, 10])).toEqual([
				4, 8, 12,
			]);
		});
		it("should return first argument", () => {
	...
	describe("interpolate", () => {
		it("should intepolate two colors", () => {
			expect(ROT.Color.interpolate([10, 20, 40], [100, 200, 300], 0.1)).toEqual(
				[19, 38, 66],
			);
		});
		it("should round values", () => {
			expect(ROT.Color.interpolate([10, 20, 40], [15, 30, 53], 0.5)).toEqual([
				13, 25, 47,
			]);
		});
		it("should default to 0.5 factor", () => {
			expect(ROT.Color.interpolate([10, 20, 40], [20, 30, 40])).toEqual([
				15, 25, 40,
			]);
		});
	});
	...
	describe("interpolateHSL", () => {
		it("should intepolate two colors", () => {
			expect(
				ROT.Color.interpolateHSL([10, 20, 40], [100, 200, 300], 0.1),
			).toEqual([12, 33, 73]);
		});
	});

	describe("randomize", () => {
		it("should maintain constant diff when a number is used", () => {
	...
	describe("rgb2hsl and hsl2rgb", () => {
		it("should correctly convert to HSL and back", () => {
			var rgb = [
				[255, 255, 255],
				[0, 0, 0],
				[255, 0, 0],
