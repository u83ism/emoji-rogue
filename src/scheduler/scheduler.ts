...
export const createSchedulerState = <T>(): SchedulerState<T> => {
	return { queue: createEventQueue<T>(), repeatList: [], current: null };
};

export const addToRepeatList = <T>(
...
export const clearSchedulerState = <T>(state: SchedulerState<T>): void => {
	state.queue.clear();
	state.repeatList = [];
	state.current = null;
};

export const removeFromSchedulerState = <T>(
	state: SchedulerState<T>,
	item: T,
): boolean => {
...
export const advanceScheduler = <T>(state: SchedulerState<T>): T | null => {
 ...
 */
export const createScheduler = <T>(): Scheduler<T> => {
