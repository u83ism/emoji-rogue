...
- **原作Rogue仕様差分の棚卸し**(2026-07-28、`/goal`セッションでの調査): 「原作Rogueに仕様をできるだけ揃える」方針のもと、現状とのギャップを分野別に洗い出した。マイルストーン100(空腹Weak段階+睡眠ガストラップ)で1件着手済み、残りは未着手のまま優先度未定でここに記録:
  ...
  - **敵ロースター**: マイルストーン101・102で原作26種全て実装済み(完了)。唯一、ハエトリソウの「隣接中はプレイヤーを拘束して逃がさない」効果はメカニクス面での見送り事項として残っている——`applyMove`(advanceTurn.ts)側に新しい制約を追加する必要があり影響範囲が広いため。着手する場合は専用のマイルストーンとして起こす
