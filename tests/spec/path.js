describe("Path", () => {
	...
	var VISITS = 0;
	var PASSABLE_CALLBACK_VISIT = (x, y) => {
		VISITS++;
		return true;
	};
	...
	describe("Dijkstra", () => {
		describe("8-topology", () => {
			...
			var dijkstra = new ROT.Path.Dijkstra(Z[0], Z[1], PASSABLE_CALLBACK_48, {
				topology: 8,
			});

			it("should compute correct path A", () => {
				dijkstra.compute(A[0], A[1], PATH_CALLBACK);
				expect(PATH.toString()).toEqual(PATH_A.toString());
			});

			it("should compute correct path B", () => {
				dijkstra.compute(B[0], B[1], PATH_CALLBACK);
				expect(PATH.toString()).toEqual(PATH_B.toString());
			});

			it("should survive non-existant path X", () => {
		...
		describe("6-topology", () => {
			...
			var dijkstra = new ROT.Path.Dijkstra(Z6[0], Z6[1], PASSABLE_CALLBACK_6, {
				topology: 6,
			});
	...
	describe("A*", () => {
		describe("8-topology", () => {
			var PATH_A = [0, 1, 0, 2, 0, 3, 1, 4, 2, 4, 3, 4, 4, 4];
			var PATH_B = [2, 2, 1, 2, 0, 3, 1, 4, 2, 4, 3, 4, 4, 4];
			var astar = new ROT.Path.AStar(Z[0], Z[1], PASSABLE_CALLBACK_48, {
				topology: 8,
			});
			...
			it("should efficiently compute path", () => {
		...
		describe("6-topology", () => {
			var PATH_A = [4, 0, 2, 0, 1, 1, 2, 2, 3, 3, 5, 3, 6, 2, 8, 2, 9, 3, 8, 4];
			var PATH_B = [10, 0, 9, 1, 8, 2, 9, 3, 8, 4];
			var astar = new ROT.Path.AStar(Z6[0], Z6[1], PASSABLE_CALLBACK_6, {
				topology: 6,
			});
