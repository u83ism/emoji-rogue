...
/** Bump on any breaking change to the GameState shape. 2: floor + stairs. 3: items. 4: inventory. 5: playerAttackDamage. 6: playerDefense. 7: playerFood. 8: goldPiles + goldCollected. 9: traps. 10: identifiedPotionKinds. 11: enemies[].awake. 12: hasRingOfRegeneration. 13: stairs.direction + amulet + hasAmulet. 14: hasRingOfSustenance. 15: confusedTurnsRemaining. 16: enemies[].slowedTurnsRemaining. 17: levitationTurnsRemaining. 18: armorProtected. 19: blindTurnsRemaining. 20: playerMaxHp + playerLevel + playerExperience. 21: paralyzedTurnsRemaining. 22: detectMonstersTurnsRemaining. 23: turnsOnCurrentFloor. 24: hasAttacked + hasEaten. 25: ItemKind renames (heal-potion, teleport-scroll, mapping-scroll, identify-scroll, regeneration-ring, sustenance-ring, striking-wand, slow-wand). 26: ItemKind rename (shield -> armor). 27: inventory stacks removed — `InventoryEntry[]` ({kind,quantity}) became plain `ItemKind[]`, one slot per held item. 28: real equipment (milestone 81) — inventory entries became `HeldItem[]` (itemId + equip/curse/bonus state for sword/armor/ring), `nextItemId` added, `playerAttackDamage`/`playerDefense`/`hasRingOfRegeneration`/`hasRingOfSustenance`/`armorProtected` removed, `playerPower` added, `ItemKind` gained `remove-curse-scroll`. 29: floor `items[]` sword/armor/ring entries gained an optional `identity` (itemId/cursed/bonus) — fixes a dropped equipped item rerolling into a fresh one on re-pickup. 30: `enemies[].confusedTurnsRemaining` added (confuse monster scroll). 31: `hallucinatingTurnsRemaining` added (hallucination potion). 32: `winds-of-kron-warning`/`winds-of-kron-eviction` removed from the GameEvent union (Winds of Kron mechanic deleted — no precedent in Rogue 5.4, see docs/design.md's コンセプト方針). `turnsOnCurrentFloor` itself is unchanged (still ticks every turn, now only for frame.ts's hallucination decoy glyph). */
