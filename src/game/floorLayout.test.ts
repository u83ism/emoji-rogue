...
describe("buildFloorLayout", () => {
	it("is deterministic: the same rng state produces the same layout", () => {
		const first = buildFloorLayout(40, 20, createRng(7), 1, "down");
		const second = buildFloorLayout(40, 20, createRng(7), 1, "down");
		expect(second).toEqual(first);
	});

	it("places the player and the staircase on floor tiles", () => {
		const layout = buildFloorLayout(40, 20, createRng(7), 1, "down");
