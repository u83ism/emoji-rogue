...
- [x] ログ設計の見直し(2026-07-14、実機フィードバック): 「セーブしました」を独立のシステム通知行からログイベント(`GameEvent`の`game-saved`、payloadなし)に変更し、戦闘ログと同じ1本の時系列に統一。セーブファイルにもイベントとして載るので**再開直後のログに「セーブした」が履歴として残る**。全角入力警告だけはゲームの出来事ではない(セーブに載せるべきでない)ため別扱いを維持し、ログとの間に1行空けて視覚的に区別
