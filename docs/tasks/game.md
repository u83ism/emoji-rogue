...
- [x] `docs/emoji-registry.md`: 敵グリフ一覧を全10種(旧版はorc以降が抜けていた既存ドリフトも合わせて解消)+新規8種に更新。アクエーターの元ネタ(初期版rust monster、D&D著作権問題での改名)とドラゴン絵文字が中華風に寄りがちな点の調査メモを追記
...
- **原作Rogue仕様差分の棚卸し**(2026-07-28、`/goal`セッションでの調査): 「原作Rogueに仕様をできるだけ揃える」方針のもと、現状とのギャップを分野別に洗い出した。マイルストーン100(空腹Weak段階+睡眠ガストラップ)で1件着手済み、残りは未着手のまま優先度未定でここに記録:
  ...
  - **敵ロースター**: マイルストーン101で「パラメータだけで差別化する」8種(ラット・エミュー・ケストレル・ホブゴブリン・ケンタウロス・クアッガ・アーヴァイル・ジャバウォック)を追加し、原作26種中18種まで到達。残り8種(グリフィン・ホビットではなくトロル=自己回復、アイシング=盲目で常に徘徊、ハエトリソウ=非移動+隣接拘束、メデューサ=視線での石化/混乱、ファントム=視界に映らない透明化、レイス=命中時に永続弱体化、ゼロック=金塊への擬態)は`advanceEnemies`に新しい分岐が要り、1種ごとに個別の設計判断が必要(グリフィン/トロルは既存のvampireLifesteal.tsに近い「命中時ではなく毎ターンの自己回復」という新パターンが要る点で地続き、他6種はそれぞれ独立した新メカニクス)
