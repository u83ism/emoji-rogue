...
- [ ] 実機スモークテスト: バンプ攻撃・敵撃破・被弾とHP減少・死亡(💀表示含む)・ログ表示・バランス感を確認。※「待機で敵が動かない」報告はロジック層で再現せず(30シード×120ターンの待機で視界内非隣接の敵は毎回接近、`toAction(" ")`→waitも正常)。隣接済みの敵は移動せずその場で攻撃する仕様。HPが減っていない場合のみ入力層の問題なので要追調査
